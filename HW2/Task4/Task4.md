...
## Оценка числа итераций через невязку и число обусловленности

Пусть $r_k = A x_k - b$ - невязка, а $e_k = x - x_k$.\
Из $A e_k = r_k$ следует $||e_k|| \le ||A^{-1}|| \cdot ||r_k||$.\
Тогда относительная ошибка оценивается через число обусловленности:

$$\frac{||e_k||}{||x||} \le \kappa(A) \frac{||r_k||}{||b||}$$

Если метод сходится со спектральным радиусом $\rho$, то невязка убывает
как $||r_k|| \le \rho^k ||r_0||$.\
Подставляя, получаем:

$$\frac{||e_k||}{||x||} \le \kappa(A) \rho^k \frac{||r_0||}{||b||}$$

Чтобы достичь точности $\varepsilon$, нужно

$$k \ge \frac{\ln(\kappa(A) \frac{||r_0||}{||b||} / \varepsilon)}{\ln(1/\rho)}$$

------------------------------------------------------------------------

## Практическая оценка числа шагов

Для $\kappa(A) = 2.57$, $\rho = max|1-\tau \lambda_i|\approx 0.44$ и $\varepsilon = 10^{-5}$
имеем:

$$k \ge \frac{\ln(2.57 / 10^{-5})}{\ln(1/0.44)} \approx 15$$
---
...
- Использование числа обусловленности позволяет **оценить число итераций**, необходимых для достижения заданной точности (при $\tau \approx 0.072$):
